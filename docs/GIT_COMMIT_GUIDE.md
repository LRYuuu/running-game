...
### 禁止使用 `git reset --hard` 撤销提交！

`git reset --hard` 会**同时撤销提交和工作区的修改**，导致未提交的代码丢失。

| 命令 | 效果 | 使用场景 |
|------|------|----------|
| `git reset --soft HEAD~1` | 撤销提交，修改保留在暂存区 | **推荐**：默认使用 |
| `git reset HEAD~1` | 撤销提交，修改保留在工作区 | **推荐**：安全操作 |
| `git reset --hard HEAD~1` | 撤销提交并丢弃修改 | **危险**：仅在确认要丢弃修改时使用 |
...
# 2. 使用安全方式撤销提交
git reset --soft HEAD~1    # 修改保留在暂存区（推荐）
# 或
git reset HEAD~1           # 修改保留在工作区
```

> **记住**：永远不要默认使用 `--hard` 参数！除非用户明确要求丢弃所有修改。
