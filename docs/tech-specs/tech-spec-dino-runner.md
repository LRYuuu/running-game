...
在现有 Square Fireline Unity 项目中实现基础 Runner 游戏机制。使用 **Unity 2D Tilemap 系统**实现程序化地形生成（多层平台、起伏地形、坑洞陷阱），使用**对象池 + 预制体**实现障碍生成（地面障碍、飞鸟）。包括玩家奔跑（逐帧动画）、二段跳跃、随时间加速、碰撞检测、得分系统和游戏结束/重新开始功能。采用像素卡通可爱美术风格，支持全平台适配（PC、移动端）。
